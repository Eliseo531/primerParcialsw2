            ...
            <form action="{{ route('tareas.update', $tarea) }}" method="POST" class="space-y-6">
                        ...
                        <select name="modulo_id"
                            ...
                            @foreach ($modulos as $modulo)
                                <option value="{{ $modulo->id }}"
                                    {{ old('modulo_id', $tarea->modulo_id) == $modulo->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <textarea name="descripcion" rows="5"
                        class="w-full rounded-xl border border-slate-300 px-4 py-3 focus:outline-none focus:ring-2 focus:ring-cyan-500">{{ old('descripcion', $tarea->descripcion) }}</textarea>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <select name="responsable_id"
                            ...
                            @foreach ($desarrolladores as $desarrollador)
                                <option value="{{ $desarrollador->id }}"
                                    {{ old('responsable_id', $tarea->responsable_id) == $desarrollador->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input type="text" value="{{ str_replace('_', ' ', ucfirst($tarea->estado)) }}"
                            class="w-full rounded-xl border border-slate-300 bg-slate-100 px-4 py-3 text-slate-500"
                            disabled>
                        <p class="text-xs text-slate-500 mt-2">
                            El estado se cambia desde el detalle de la tarea.
                        </p>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-4 pt-4">
                    ...
                    <a href="{{ route('tareas.show', $tarea) }}"
                        class="inline-flex items-center rounded-xl bg-slate-800 px-6 py-3 text-white font-semibold hover:bg-slate-700 transition">
                        Ver detalle
                    </a>
                </div>
            </form>
