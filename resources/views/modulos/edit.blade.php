...
@section('content')
    <div class="max-w-4xl mx-auto">
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-slate-800">Editar módulo</h1>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
            <form action="{{ route('proyectos.modulos.update', [$proyecto, $modulo]) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')

                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2">Nombre del módulo</label>
                    <input type="text" name="nombre" value="{{ old('nombre', $modulo->nombre) }}"
                ...
                <div>
                    ...
                    <textarea name="descripcion" rows="5"
                        class="w-full rounded-xl border border-slate-300 px-4 py-3 focus:outline-none focus:ring-2 focus:ring-cyan-500">{{ old('descripcion', $modulo->descripcion) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <select name="estado"
                        class="w-full rounded-xl border border-slate-300 px-4 py-3 focus:outline-none focus:ring-2 focus:ring-cyan-500"
                        required>
                        <option value="activo" {{ old('estado', $modulo->estado) === 'activo' ? 'selected' : '' }}>Activo
                        </option>
                        <option value="inactivo" {{ old('estado', $modulo->estado) === 'inactivo' ? 'selected' : '' }}>
                            Inactivo</option>
                    </select>
                </div>
